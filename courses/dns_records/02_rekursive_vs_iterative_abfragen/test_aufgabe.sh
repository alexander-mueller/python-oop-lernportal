...
# Führe Funktion aus
trace_dns_resolution "example.com" "$TEST_DIR"
RET=$?

if [ $RET -eq 0 ]; then
  echo -e "${GREEN}✓ Test 1: Funktion trace_dns_resolution existiert und gibt 0 zurück.${NC}"
  ...
  echo -e "${GREEN}🎉 ERFOLG: Alle DNS-Prüfungen in DNS 02: Rekursive vs. Iterative Abfragen bestanden!${NC}"
  exit 0
else
