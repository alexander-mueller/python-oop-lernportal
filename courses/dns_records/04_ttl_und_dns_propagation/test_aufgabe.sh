...
# Führe Funktion aus
calculate_ttl_migration "example.com" "$TEST_DIR"
RET=$?

if [ $RET -eq 0 ]; then
  echo -e "${GREEN}✓ Test 1: Funktion calculate_ttl_migration existiert und gibt 0 zurück.${NC}"
  ...
  echo -e "${GREEN}🎉 ERFOLG: Alle DNS-Prüfungen in DNS 04: TTL & DNS-Propagation bestanden!${NC}"
  exit 0
else
