...
# Führe Funktion aus
analyze_dns_hierarchy "example.com" "$TEST_DIR"
RET=$?

if [ $RET -eq 0 ]; then
  echo -e "${GREEN}✓ Test 1: Funktion analyze_dns_hierarchy existiert und gibt 0 zurück.${NC}"
  ...
  echo -e "${GREEN}🎉 ERFOLG: Alle DNS-Prüfungen in DNS 01: DNS-Hierarchie & Root-Server bestanden!${NC}"
  exit 0
else
